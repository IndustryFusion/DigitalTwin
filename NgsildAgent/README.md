...
autonumber
   ...
   Participant Device;
   Participant MQTT;
   Admin ->> Keycloak: Request Vanilla Onboarding Token
   Keycloak ->> Admin: Return Vanilla Onboarding Token
   Admin ->> Device: Provide Vanilla Onboarding Token
   Device ->> Keycloak: Activate by Refreshing and providing device id in HTTP Header
   Keycloak ->> Device: Provide Device Token and Refreshed Token with "device_id" as claim
