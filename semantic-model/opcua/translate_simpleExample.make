...
ONTOLOGY_NS 		:= http://example.org/
INSTANCE_TYPE 		:= http://example.org/AlphaType
...
# Version and source NodeSet URLs
# -----------------------------------------------------------------------------
NODESET_DIR := ./docs/files

INSTANCE_NODESET          	:=   ${NODESET_DIR}/Example.NodeSet2.xml

# Dependent Ontology URLs (or local files if not in remote mode)
CORE_ONTOLOGY                       := $(OPCUA_PREFIX)core.owl.ttl
DI_ONTOLOGY                    		:= $(OPCUA_PREFIX)di.owl.ttl
...
INSTANCE_ONTOLOGY          		:=  example.owl.ttl
INSTANCE_OPTS					:= -p example -n $(ONTOLOGY_NS)
INSTANCE_JSONLD_OPTS			:= -n $(INSTANCE_IRI)
INSTANCE_TYPE					:= $(INSTANCE_TYPE)
INSTANCE_DEPENDENCIES      		= $(CORE_ONTOLOGY) 
...
shacl.ttl instances.jsonld entities.ttl context.jsonld $(INSTANCE_ONTOLOGY): $(INSTANCE_DEPENDENCIES)
	...
	python3 owl2instances.py -t $(INSTANCE_TYPE) -i "sn1234" -sb -n $(INSTANCE_NS_PREFIX) $(INSTANCE_ONTOLOGY)
