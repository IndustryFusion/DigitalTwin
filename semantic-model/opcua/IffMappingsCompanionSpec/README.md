...
The machine defines an Object `ProprietaryDataObject1` which can be browsed and exposes the variables throught the OPC UA Server.
...
* **LogicExpression**: Expression to transform the variables in SPARQL. This must provide only the `WHERE` clause of the SPARQL query. The follwing variables can be defined in the query:
    ...
    * ?trustLevel: Value between 0 `no-trust` an 1 `trust`from the connector point of view. e.g. if values are missing or not plausible, the connector can still forward but let the Digital Twin know how much it trusts the value.
...
In this section, we will walk through an example. The example consist of the following files:
