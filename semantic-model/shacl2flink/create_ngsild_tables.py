...
def main(output_folder='output'):
    ...
    with open(os.path.join(output_folder, "ngsild.yaml"), "w") as f, \
        ...
        yaml.dump(utils.create_kafka_topic(f'{configs.kafka_topic_constraint_table_object}',
                                           f'{configs.kafka_topic_constraint_table_name}', configs.kafka_topic_object_label,
                                           config),
                  fk)
        ...
        yaml.dump(utils.create_kafka_topic(f'{configs.kafka_topic_constraint_trigger_table_object}',
                                           f'{configs.kafka_topic_constraint_trigger_table_name}', configs.kafka_topic_object_label,
                                           config),
                  fk)
        ...
        connector = 'upsert-kafka'
        print('---', file=f)
        yaml.dump(utils.create_constraint_combination_yaml_table(connector, kafka_constraint_combination_checks, value), f)
        ...
        yaml.dump(utils.create_kafka_topic(f'{configs.kafka_topic_constraint_combination_table_object}',
                                           f'{configs.kafka_topic_constraint_combination_table_name}', configs.kafka_topic_object_label,
                                           config),
                  fk)
