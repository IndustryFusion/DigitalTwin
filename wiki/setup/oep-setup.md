...
3. Start installing using [helm installtion instruction of hosted k8s](https://github.com/yshashix/DigitalTwinScorpio/tree/main/helm) using oep.yaml environment and dockerhub access. Do following steps from 4 on another console after namespaces are created as steps from 4 is used to make it succesfully installed. 
   ```
   git checkout v0.7.0
   cd ../test
...
5. Configure edge-dns or rke2-dns configmanagement Corefile for your domain name added as per oep environment file [oep-testing.industry-fusion.com], see test/setup-local-ingress.sh for idea
   ...
   - add in Corefile as below, mark addition after ready  till fallthrough your domain name with ingress external ip
